...
def calculate_risk(liquidity, volume):
    ...
    elif liquidity > 50000: score += 30
    elif liquidity > 10000: score += 20
    else: score += 5

    if volume > 100000: score += 40
    elif volume > 50000: score += 30
    elif volume > 10000: score += 20
    else: score += 5

    if score >= 70: verdict = "LOW RISK SAFE"
    elif score >= 40: verdict = "MODERATE RISK"
    else: verdict = "HIGH RISK WARNING"
    return score, verdict

def detect_pump(liquidity, volume):
    ratio = volume / liquidity if liquidity > 0 else 0
    if ratio > 1: return "POSSIBLE PUMP DETECTED"
    elif ratio > 0.5: return "HIGH ACTIVITY"
    else: return "NORMAL ACTIVITY"

async def handle_token(update: Update, context: ContextTypes.DEFAULT_TYPE):
    token = update.message.text.strip()
    user_tokens[update.effective_user.id] = token
    await update.message.reply_text("Token detected. Select analysis:", reply_markup=get_keyboard())
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif query.data == "risk": text = f"Risk Score: {score}/80\nVerdict: {verdict}"
    elif query.data == "pump": text = f"Pump Status: {pump}"
    elif query.data == "price": text = f"Price: ${data['price']}"
    elif query.data == "liquidity": text = f"Liquidity: ${data['liquidity']}"
    elif query.data == "volume": text = f"Volume: ${data['volume']}"
...
def main():
    ...
    print("TokenAnalyzer Pump Detection LIVE & Web Server Started")
    app.run_polling()
